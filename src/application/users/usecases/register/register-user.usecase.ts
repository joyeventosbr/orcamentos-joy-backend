...
import { makeRegisterUserSchema } from "./register-user.dto";
...
export class RegisterUserUseCase {
  ...
  async execute(input: unknown): Promise<Result<RegisterUserResponse>> {
    const resultData = makeRegisterUserSchema(this.validationService).safeParse(
      input,
    );
    if (resultData.error) {
    ...
    const newUser = User.create({
      ...
      cdEmpresa: resultData.data.cdEmpresa,
    });
